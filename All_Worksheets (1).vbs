...
rem             sheet.Rows(1).delete  ' this will remove Row 1 or the header Row
		...
		Case "Reps"
			sheet.SaveAs strScriptPath & "us_rep_quotas" & ".csv", xlCSV, local 'CSV
		Case "ASDs"
			sheet.SaveAs strScriptPath & "us_quota_file_asd" & ".csv", 6, xlCSV, local 'CSV
		Case "Districts"
			sheet.SaveAs strScriptPath & "us_quota_file_dist" & ".csv", 6, xlCSV, local 'CSV
		Case Else
			sheet.SaveAs strScriptPath & "us_quota_file_dist" & ".csv", 6, xlCSV, local 'CSV
		End Select
