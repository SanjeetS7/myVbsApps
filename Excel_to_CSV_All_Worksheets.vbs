...
strExcelFileName = "Quota Load Template 2017 06-29-17" 'file name to parses
...
rem             sheet.Rows(1).delete  ' this will remove Row 1 or the header Row
		...
		if sheet.Name = "Reps" Then
			sheet.SaveAs strScriptPath & "us_rep_quotas" & ".csv", 6 'CSV
		else If sheet.Name = "ASDs" then
				sheet.SaveAs strScriptPath & "us_quota_file_asd" & ".csv", 6 'CSV
		end if
			else 
				sheet.SaveAs strScriptPath & "us_quota_file_dist" & ".csv", 6 'CSV
